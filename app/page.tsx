    ...
    <main>
      <header className="sticky top-0 z-20 border-b border-forest-900/10 bg-cream/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          ...
          <a
            href="#tasks"
            className="rounded-full bg-forest-900 px-4 py-2 text-sm font-bold text-white"
          >
            Get involved
          </a>
        </div>
      ...
      <section className="overflow-hidden px-5 py-16 md:py-24">
        <div className="mx-auto grid max-w-6xl items-center gap-12 md:grid-cols-[1.05fr_.95fr]">
          <div>
            <p className="font-bold uppercase tracking-[0.3em] text-earth">
              For the builders
            </p>
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-forest-700">
              Discover a food forest in Eindhoven, help with practical work,
              and learn what the season is teaching.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href="#forests"
                className="flex items-center gap-2 rounded-full bg-forest-900 px-6 py-3 font-bold text-white"
              >
                Find a forest
                <ArrowRight size={18} />
              </a>
              ...
              <a
                href="#tasks"
                className="rounded-full border border-forest-900 px-6 py-3 font-bold"
              >
                Browse tasks
              </a>
            </div>
      ...
      <section id="forests" className="bg-white px-5 py-20">
        <div className="mx-auto max-w-6xl">
          <p className="font-bold uppercase tracking-[0.25em] text-earth">
            Explore
          </p>

          <h2 className="mt-2 text-4xl font-black">
            Find a forest in Eindhoven
          </h2>

          <p className="mt-3 max-w-2xl text-forest-700">
            Choose a local food forest, see where it is located, and discover
            current volunteer tasks and seasonal harvest information.
          </p>
          ...
          <div className="mt-7 grid gap-5 md:grid-cols-2">
            {forests.map((forest) => (
              <article
                key={forest.id}
                className="flex h-full flex-col overflow-hidden rounded-3xl border border-forest-900/10 bg-white shadow-soft"
              >
                <Image
                  src={forest.image}
                  width={700}
                ...
                <div className="flex flex-1 flex-col p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 rounded-full bg-forest-100 px-3 py-1 text-xs font-bold">
                      {forest.status}
                    </span>
                  </div>
                  ...
                  <p className="mt-3 flex-1 text-sm leading-6 text-forest-700">
                    {forest.description}
                  </p>

                  <a
                    href={`https://www.google.com/maps/dir/?api=1&destination=${forest.latitude},${forest.longitude}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={`Get directions to ${forest.name}`}
                    className="mt-5 inline-flex w-fit items-center gap-2 rounded-full bg-forest-900 px-4 py-2 text-sm font-bold text-white transition hover:bg-forest-700"
                  >
                    Get directions
      ...
      <section id="harvest" className="mx-auto max-w-6xl px-5 py-20">
        ...
        <h2 className="mt-2 text-4xl font-black">
          What the forest is teaching now
        </h2>

        <p className="mt-3 max-w-2xl text-forest-700">
          Learn what is ready to harvest, what should be left to grow, and how
          different plants support a healthy food forest ecosystem.
        </p>
        ...
        <div className="mt-9 grid gap-5 md:grid-cols-3">
          {harvests.map((item) => (
            ...
            >
              ...
              <div className="p-6">
                <span className="rounded-full bg-forest-100 px-3 py-1 text-xs font-bold">
                  {item.status}
                </span>
        ...
        <div className="mx-auto flex max-w-6xl flex-col justify-between gap-3 md:flex-row">
          <div>
            ...
            <p className="mt-1 text-sm text-forest-700">
              An open-source project supported by GDG Eindhoven.
            </p>
          </div>

          <div className="text-sm text-forest-700">
            Built to connect communities with local food forests through
            volunteering, seasonal harvests and shared learning.
          </div>
        </div>
      </footer>
    </main>
